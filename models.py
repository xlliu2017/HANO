...
class DilResNet(nn.Module):
    def __init__(self, in_channels=1, hid_channels=48, out_channels=1, stride=1):
        ...
        """
        Baseline DilResNet. Implemented according to the paper https://openreview.net/forum?id=msRBojTz-Nh    
        """
...
class SpectralConv2d_FNO(nn.Module):
    ...
    def forward(self, x):
        batchsize = x.shape[0]
        # Compute Fourier coeffcients up to factor of e^(- something constant)
        x_ft = torch.fft.rfft2(x)  # 离散傅里叶变换
...
class FNO2d(nn.Module):
    def __init__(self, R_dic):
        ...
        self.modes1 = R_dic['FNO_modes']
        self.modes2 = R_dic['FNO_modes']
        self.width = R_dic['feature_dim']
        ...
        self.conv4 = SpectralConv2d_FNO(self.width, self.width, self.modes1, self.modes2)
        self.w0 = nn.Conv2d(self.width, self.width, 1)  # 卷积核为1，W疑似为对角矩阵！
